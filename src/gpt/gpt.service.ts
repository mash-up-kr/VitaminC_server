  ...
  async recommendRestaurants(
    ...
    try {
      ...
      if (response.choices[0].finish_reason === 'tool_calls') {
        ...
        const prompt = `
      제공된 식당 정보는 ${user.nickname}님의 요구와 선호도에 맞춰 추천된 식당들입니다.
      해당 식당들은 ${user.nickname}님의 선호도에 맞춰 추천됐음을 언급해주세요.
      요구와 선호도를 바탕으로 공통된 추천 이유를 간단히 설명해주세요.
      식당 정보는 따로 언급하지 않고, 식당들의 추천 이유를 짧게 설명해주세요.
      그리고 친절한 존댓말로 2줄 이내로 답변해 주세요.
